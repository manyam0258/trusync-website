...
import SentinelPrivacyWidget from "@/components/SentinelPrivacyWidget";
...
export function Contact() {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      if (result.success) {
        setSealedReceipt(result as SentinelReceipt);
        // 2. Proceed with standard CRM/backend ingestion (existing Supabase call)
        const crmResult = await submitContactForm(formData);
        if (crmResult.error) {
          showToast(crmResult.error, "error");
        } else {
          showToast('Enquiry submitted and cryptographically sealed under DPDP Act!', "success");
      ...
      console.error('Sentinel submission error:', err);
      showToast('Error submitting enquiry: ' + err.message, "error");
    } finally {
      setSubmitting(false);
    }
  };
                ...
                <div className="space-y-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <a href="tel:+918****3438" className="text-white/90 hover:text-white">+91 81434 83438</a>
                    </div>
                  </div>
            ...
            <div className="lg:col-span-3 p-10">
              <form action={handleSubmit} id="contact-form" className="space-y-6">
                ...
                <Button type="submit" size="lg" className="w-full sm:w-auto h-12 px-8 rounded-xl shadow-lg shadow-primary/20 hover:shadow-primary/30 hover:scale-105 transition-all">
                  {submitting ? 'Submitting & Cryptographically Sealing...' : 'Send Enquiry'}
                  <Send className="ml-2 h-5 w-5" />
                </Button>

                {/* Confirmation of Non-Repudiation Receipt */}
                {sealedReceipt && (
                  <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg text-sm">
                    <strong>✅ Compliance Proof Sealed:</strong>
                    <div className="mt-2">Principal ID: <code>{sealedReceipt.principal_id || 'N/A'}</code></div>
                    {sealedReceipt.consent_receipt && (
                      <div className="mt-1">Proof Hash: <code>{sealedReceipt.consent_receipt.proof_hash?.substring(0, 24) || 'N/A'}...</code></div>
                    )}
                  </div>
                )}
              </form>
